...
function TrustedBy() {
  const partners = [
    ...
    { name: 'Ракурс-инжиниринг', desc: 'Ведущий производитель оборудования для автоматизации гидро, тепло и атомных станций', logo: 'mp5horfy-photo_2026-05-14_19-52-30.jpg' },
    ...
    <section id="process">
      <div className="container">
        ...
        <div className="process-grid">
          {steps.map((s, i) => (
            <div key={s.t} className="card card-sm" style={{
              padding: 28,
              background: i === 0 ? 'var(--c-canvas)' : 'var(--c-white)',
              color: 'var(--c-ink)',
              border: i === 0 ? 'none' : '1px solid var(--c-faded)',
              minHeight: 200,
              display: 'grid', gridTemplateRows: '20px 32px 1fr', gap: 12,
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <span style={{
                  fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: 0.08,
              ...
              <div style={{ fontSize: 22, fontWeight: 600, letterSpacing: '-0.02em', alignSelf: 'start' }}>{s.t}</div>
              <p style={{
                margin: 0, fontSize: 14, lineHeight: 1.5,
                color: 'var(--c-graphite)', alignSelf: 'start'
              }}>{s.d}</p>
            </div>
          ))}
        </div>
        ...
        <div className="card" style={{
          background: 'var(--c-ink)', color: 'var(--c-white)',
          padding: 48, marginTop: 24
        }}>
          <div style={{ display: 'flex', gap: 32, alignItems: 'stretch' }} className="fleet-feature-grid">
            {/* Left: Content */}
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
              ...
              <ul style={{ margin: '28px 0 0', padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 14 }}>
                {[
                  ['Фотоотчёт на всех этапах', <IcoCamera key="c" size={18} />],
                  ['Температурный график с термописца', <IcoThermo key="t" size={18} />],
                  ['Все документы в электронном виде', <IcoDoc key="d" size={18} />],
                ].map(([txt, ico]) => (
                  <li key={txt} style={{
                    display: 'flex', alignItems: 'center', gap: 14,
                    padding: 18, borderRadius: 20,
      ...
      <div className="container">
        <div className="sec-head">
          <div>
            ...
            <div className="eyebrow" style={{ marginTop: 8 }}>Стоимость</div>
            <h2 className="display-sm">Индивидуальный расчёт стоимости.</h2>
          </div>
        </div>
      ...
      <div className="container">
        <div className="sec-head">
          <div>
            ...
            <div className="eyebrow" style={{ marginTop: 8 }}>Контакты</div>
            <h2 className="display-sm">Свяжитесь с Термологистикой.</h2>
          </div>
          <p className="body-lg">
    ...
    <footer>
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gap: 48 }} className="footer-grid">
          <div>
            <div className="logo" style={{ color: 'var(--c-white)' }}>
              <span className="logo-mark" />
              <span>Термологистика</span>
            </div>
