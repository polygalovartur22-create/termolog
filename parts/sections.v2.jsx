    ...
    <nav className="nav">
      <div className="container nav-inner">
        ...
        <div className="nav-cta" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <a href="tel:+79214195500" className="btn btn-ghost btn-sm nav-phone" style={{ display: 'inline-flex' }}>
            <IcoPhone size={14} /> <span className="nav-phone-text">+7 (921) 419-55-00</span>
          </a>
      ...
      <div className="nav-mobile-menu">
          ...
          <a href="#cases" onClick={() => setMobileOpen(false)}>Кейсы</a>
        ...
        <div style={{
          ...
          <div>
            ...
            <div className="hero-btns" style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              ...
              <a href="https://wa.me/79214195500" className="btn" style={{ background: 'var(--c-wa)', color: 'var(--c-white)' }}>
        ...
        <div className="card step7-card" style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'stretch' }} className="step7-grid">
            ...
            <div className="photo-frame" style={{
              borderRadius: 24, overflow: 'hidden',
              border: '1px solid rgba(255,255,255,0.1)',
              minHeight: 240, minWidth: 0, height: "605px"
            }}>
              <img
                src="photo-hero-3.jpg"
                alt="Курьер и клиент"
                style={{ width: '100%', objectFit: 'cover', objectPosition: 'center top', height: "606px" }} />
...
function CostForm() {
  ...
  const [form, setForm] = React.useState({
    name: '', contactMethod: 'Звонок', contact: '',
    cargo: 'Цветы', weight: '', route: '', regime: '', comment: '', agree: false
  });
  const set = (k) => (e) => setForm({ ...form, [k]: e.target.value });

  const isTelegram = form.contactMethod === 'Telegram';
  const contactConfig = isTelegram ?
  {
    type: 'text',
    placeholder: '@username',
    pattern: '@?[A-Za-z0-9_]{4,32}',
    inputMode: 'text',
    title: 'Telegram username: 4–32 символа, латиница / цифры / «_»'
  } :
  {
    type: 'tel',
    ...
    pattern: '\\+?[0-9\\s\\-\\(\\)]{10,20}',
    inputMode: 'tel',
    title: 'Номер телефона — можно в международном формате с «+»'
  };

  const switchMethod = (method) => {
    // Клирим введённое значение при переключении в / из Telegram — форматы несовместимы
    const goingToTg = method === 'Telegram';
    const wasTg = form.contactMethod === 'Telegram';
    setForm({
      ...form,
      contactMethod: method,
      contact: goingToTg !== wasTg ? '' : form.contact
    });
  };

  const contactMethods = ['Звонок', 'Telegram', 'Max'];
    ...
    <section id="calc">
      <div className="container">
        ...
        <div className="card calc-card cost-form-grid" style={{
          ...
          <form onSubmit={(e) => {
            e.preventDefault();
            if (!form.agree) { setAgreeError(true); return; }
            setAgreeError(false);
            setSubmitted(true);
          }}
          style={{
            ...
            display: 'flex', flexDirection: 'column', gap: 12
              ...
              {/* Contact method picker + dynamic input */}
              <FormField label={`Способ связи · ${form.contactMethod}`} required>
                <div className="contact-method-row" role="radiogroup" aria-label="Способ связи">
                ...
                <button
                  ...
                  className={`contact-method-btn${form.contactMethod === m ? ' active' : ''}`}
              ...
              <label className={`form-consent${agreeError ? ' form-consent-error' : ''}`}>
                <input
                  type="checkbox"
                  checked={form.agree}
                  onChange={(e) => { setForm({ ...form, agree: e.target.checked }); if (e.target.checked) setAgreeError(false); }}
                />
                <span>
                  Я ознакомлен(а) с <a href="privacy-policy.html" target="_blank" rel="noopener noreferrer">Политикой обработки персональных данных</a> и даю <a href="privacy-policy.html" target="_blank" rel="noopener noreferrer">согласие на обработку моих персональных данных</a>.
                </span>
              </label>
              {agreeError &&
                <div className="form-consent-msg" role="alert">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }} className="contact-grid">
          <div className="card card-canvas" style={{ padding: 40 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 24 }}>
              <ContactLine label="+7 (921) 419-55-00" sub="Телефон" href="tel:+79214195500" icon={<IcoPhone size={16} />} />
              <ContactLine label="+7 (921) 419-55-00" sub="WhatsApp" href="https://wa.me/79214195500" icon={<IcoMessage size={16} />} />
              <ContactLine label="г. Санкт-Петербург, ул. Народная, 59" sub="Адрес" icon={<IcoMail size={16} />} />
            </div>
            <div style={{ marginTop: 24, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <a href="https://wa.me/79214195500" className="btn btn-sm" style={{ background: 'var(--c-wa)', color: 'var(--c-white)' }}>
                <IcoMessage size={14} /> WhatsApp
              </a>
              <a href="tel:+79214195500" className="btn btn-ghost btn-sm">
      ...
      <div className="container">
        <div className="sec-head">
          <div>
            ...
            <h2 className="display-sm">Реальные рейсы.<br />Реальный холод.</h2>
          </div>
          <p className="body-lg">
            Не маркетинговые истории, а инженерные решения из практики —
            температурные режимы, маршруты, аэропортовые склады, дедлайны.
          </p>
        </div>

        <div className="cases-cards-list">

          {/* Hero featured card — Case 01 */}
          <article className="cc-hero">
            <div>
              <div className="cc-tag-row">
                <span className="cc-num">КЕЙС 01</span>
                <span className="cc-tag">Фарма · −20 °C / до −80 °C в коробах</span>
              </div>
              <h3 className="cc-title">«Минус 80» на трассе М-11</h3>
              <p className="cc-desc">
                Доставили термочувствительный фармгруз из «Москва Карго» с гибридной заморозкой.
                Рефрижератор, сухой лёд и термобоксы создали буферный кокон сверхнизких температур
                вокруг каждой упаковки.
              </p>
              <a href="cases.html#minus-80" className="cc-cta">
                Читать полностью <IcoArrow size={14} />
              </a>
            </div>
            <div className="cc-meta">
              <div className="row"><div className="k">Маршрут</div><div className="v">Москва Карго → СПб</div></div>
              <div className="row"><div className="k">Режим</div><div className="v">−20 °C / до −80 °C</div></div>
              <div className="row"><div className="k">Результат</div><div className="v">0 замечаний</div></div>
            </div>
          </article>

          {/* Two small cards — Cases 02 + 03 */}
          <div className="cc-row">
            <article className="cc-mini">
              <span className="cc-num">КЕЙС 02</span>
              <h3 className="cc-title">Мульти-пик с «Фармстандартом»</h3>
              <p className="cc-desc">
                Самый непредсказуемый склад поставили последним. Физика кузова против бюрократии:
                60–70&nbsp;% уже холодного груза удержали +2…+8&nbsp;°C под длительной загрузкой.
              </p>
              <div className="cc-mini-meta">
                <div className="row"><span className="k">Маршрут</span><span className="v">Москва → СПб</span></div>
                <div className="row"><span className="k">Режим</span><span className="v">+2 … +8 °C</span></div>
                <div className="row"><span className="k">Финиш</span><span className="v">«Фрезениус» + 3</span></div>
              </div>
              <a href="cases.html#multi-pick" className="cc-cta">
                Читать полностью <IcoArrow size={14} />
              </a>
            </article>

            <article className="cc-mini">
              <span className="cc-num">КЕЙС 03</span>
              <h3 className="cc-title">Орхидеи на грани</h3>
              <p className="cc-desc">
                Машина под редкий режим +14…+16&nbsp;°C найдена за 3 часа. Партия принята
                в Петербурге за 40 минут до дедлайна. Ровная полка +15&nbsp;°C без пиков и провалов.
              </p>
              <div className="cc-mini-meta">
                <div className="row"><span className="k">Маршрут</span><span className="v">Москва → СПб</span></div>
                <div className="row"><span className="k">Режим</span><span className="v">+14 … +16 °C</span></div>
                <div className="row"><span className="k">Окно</span><span className="v">3 часа</span></div>
              </div>
              <a href="cases.html#orchids" className="cc-cta">
                Читать полностью <IcoArrow size={14} />
              </a>
            </article>
          </div>
        ...
        <div className="cases-footer">
          <a href="cases.html" className="btn btn-primary">
            Смотреть все кейсы <IcoArrow size={14} />
          </a>
        </div>
      </div>
    </section>);
    ...
    <footer>
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gap: 48 }} className="footer-grid">
          ...
          <FooterCol title="Контакты" items={['+7 (921) 419-55-00', 'WhatsApp', 'Санкт-Петербург, ул. Народная, 59']} />
        </div>
        <div className="footer-bottom" style={{
          ...
          <div className="footer-socials" style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <a href="https://vk.com/termolog" target="_blank" rel="noopener noreferrer" aria-label="VK" className="social-link">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M12.785 16.241s.288-.032.435-.193c.135-.148.131-.426.131-.426s-.019-1.303.588-1.496c.598-.19 1.366 1.265 2.18 1.825.616.423 1.084.331 1.084.331l2.182-.03s1.141-.071.6-.972c-.044-.074-.315-.665-1.62-1.879-1.366-1.273-1.183-1.066.462-3.262 1.001-1.337 1.402-2.153 1.277-2.503-.119-.333-.857-.245-.857-.245l-2.457.015s-.182-.025-.317.056c-.132.08-.217.265-.217.265s-.388 1.036-.906 1.917c-1.091 1.858-1.527 1.956-1.706 1.841-.415-.268-.311-1.079-.311-1.655 0-1.799.272-2.55-.531-2.744-.267-.064-.464-.107-1.146-.114-.876-.009-1.617.003-2.036.208-.279.137-.495.441-.364.459.162.022.531.1.727.367.252.345.243 1.118.243 1.118s.146 2.137-.341 2.402c-.333.182-.791-.189-1.764-1.875-.498-.863-.875-1.817-.875-1.817s-.072-.177-.202-.272c-.156-.115-.376-.151-.376-.151l-2.336.015s-.351.01-.479.163c-.114.136-.009.418-.009.418s1.83 4.281 3.902 6.439c1.9 1.978 4.057 1.848 4.057 1.848h.976z" />
              </svg>
            </a>
            <a href="https://t.me/termologic" target="_blank" rel="noopener noreferrer" aria-label="Telegram" className="social-link">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          ...
          <div style={{ display: 'flex', gap: 24 }}>
            ...
            <a href="cookie-consent.html">Согласие на обработку cookie</a>
          </div>
        </div>
      </div>
    </footer>);
...
Object.assign(window, {
  ...
  Process, Cases, CostForm, Contact, Footer
});